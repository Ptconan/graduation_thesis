...
for i=1:nm
    if af(i) > 1.0*10^(-4)
        plot(mem0(:,1,i), mem0(:,2,i), 'k-', 'LineWidth',af(i));
    elseif af(i) < -10^(-4)
...
% h(1) = plot(coord_x((nx+1)*(ny+1),1), coord_x((nx+1)*(ny+1),2), 'ok',...
%     'MarkerFaceColor', 'r', 'MarkerSize',16); % Non-struct top-right
h(1) = plot(coord_x(nx/2+1,1), coord_x(nx/2+1,2), 'ok',...
    'MarkerFaceColor', 'r', 'MarkerSize',16); % Non-struct bottom-center
h(2) = plot(coord_x(nx+1,1), coord_x(nx+1,2), 'ok',...
    'MarkerFaceColor', 'k', 'MarkerSize',16);

legend(h([1,2]),{'Mass','Fixed'},'interpreter','latex','Location','northeast')
set(gca,'FontSize',28)
ax = gca;
ax.FontSize = 28;
